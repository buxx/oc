...
fn projectiles(w: &WorldConfig, tiles: &Vec<Tile>) -> Vec<Projectile> {
    let positions = vec![(
        UVec2::new(50, 30),
        Meters(1.0),
        UVec2::new(30, 30),
    ...
    let px_per_tile = w.geo_pixels_per_tile;
    let px_per_meter = w.geo_pixels_per_meters;
    let meters_per_z = w.geo_meters_per_z.0;

    positions
        .into_iter()
        .map(|(start, start_plus_z, end, end_plus_z)| {
            let start_tile_xy = TileXy(Xy(start.x as u64, start.y as u64));
            let start_tile_i = WorldTileIndex::from_(start_tile_xy, w);
            let start_tile = &tiles[start_tile_i.0 as usize];
            let end_tile_xy = TileXy(Xy(end.x as u64, end.y as u64));
            let end_tile_i = WorldTileIndex::from_(end_tile_xy, w);
            let end_tile = &tiles[end_tile_i.0 as usize];
            //
            let start_x = ((start_tile_xy.0.0 * px_per_tile) + px_per_tile / 2) as f32;
            let start_y = ((start_tile_xy.0.1 * px_per_tile) + px_per_tile / 2) as f32;
            let start_y = start_y.to_gui_y(w);
            let start_z = (start_tile.z as f32 * meters_per_z + start_plus_z.0) * px_per_meter;
            let start = [start_x, start_y, start_z];
            let start_ = Vec3::new(start_x, start_y, start_z);
            let end_x = ((end_tile_xy.0.0 * px_per_tile) + px_per_tile / 2) as f32;
            let end_y = ((end_tile_xy.0.1 * px_per_tile) + px_per_tile / 2) as f32;
            let end_y = end_y.to_gui_y(w);
            let end_z = (end_tile.z as f32 * meters_per_z + end_plus_z.0) * px_per_meter;
            let end_ = Vec3::new(end_x, end_y, end_z);
            let direction = (end_ - start_).normalize_or_zero();
            let translation = Force::Translation(direction.into(), MetersSeconds(5.0));
            let forces = vec![translation];
            //
            Projectile::Bullet(Bullet::new(start, forces, w))
        })
        .collect()
}
